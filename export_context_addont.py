...
class AddonPreferences(bpy.types.AddonPreferences):
	...
	data_dir : StringProperty(name="Directory: Editors' json files", default="temp", subtype='FILE_PATH')
	out_dir : StringProperty(name="Directory: Combined json file", default="outcome", subtype='FILE_PATH')
	name : StringProperty(name="Combined Json File Name", default="Cubeを選択.json")

	def draw(self, context):
		lout = self.layout
		lout.use_property_split = True
		for p in ['data_dir', 'out_dir', 'name']:
			lout.prop(self, p)
		lout.operator(CombineJsonOperator.bl_idname).name = self.name
...
class ExportContextOperator(bpy.types.Operator):
	...
	def execute(self, context):
		...
		file_path = Path.cwd() / prefs.data_dir / file_name
...
class CombineJsonOperator(bpy.types.Operator):
	...
	def execute(self, context):
		prefs = bpy.context.preferences.addons[__name__].preferences
		data_dir = Path.cwd() / prefs.data_dir
		...
		out_path = Path.cwd() / prefs.out_dir / f"{self.name}"
		with out_path.open("w", encoding='utf-8') as fw:
			json.dump(main_dic, fw, ensure_ascii=False)

		self.report({'INFO'}, f"Combine {len(json_paths)} files to {self.name}")
		return {'FINISHED'}


def menu_func(self, context):
    self.layout.operator(ExportContextOperator.bl_idname, text="CTX", icon='EXPORT')
